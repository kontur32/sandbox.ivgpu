  ...
  let $таблицаТемыПоСеместрам :=
    ...
    return
      ...
      <row>
        ...
        { $j }
        <cell>{ round( $i[ кодВидаРабот/text() = '101' ]/часы/text() div count( $темыСеместра ), 1 ) }</cell>
        <cell>{ round( $i[ кодВидаРабот/text() = '102' ]/часы/text() div count( $темыСеместра ), 1 ) }</cell>
        <cell>{ round( $i[ кодВидаРабот/text() = '103' ]/часы/text() div count( $темыСеместра ), 1 ) }</cell>
        <cell>{ round( $i[ кодВидаРабот/text() = '107' ]/часы/text() div count( $темыСеместра ), 1 ) }</cell>
        <cell>{ round( $i[ кодВидаРабот/text() = '108' ]/часы/text() div count( $темыСеместра ), 1 ) }</cell>
        <cell>
