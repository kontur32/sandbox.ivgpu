  ...
  )
{
  ...
  let $семестрыБезКонтроля := практические:семестрБезОценки( $дисциплина )
  
  let $таблицаТемыПоСеместрам :=
    for $i in $дисциплина/видыРабот/видРабот[ if( $семестрыБезКонтроля )then( семестр != $семестрыБезКонтроля )else( true() ) ]
...
declare function практические:семестрБезОценки( $дисциплина as element( Дисциплина ) ){
