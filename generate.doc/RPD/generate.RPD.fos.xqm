...
declare 
  %rest:path( '/sandbox/ivgpu/api/v01/generate/ФОС.Титул/{ $ID }/{ $кодДисциплины }' )
  ...
  let $автор :=
    content:getContent(
      ...
      (
        ...
        [ 'ПО', 'table' ],
        if( $дисциплина//кодВидаРабот/text() = '5' )
        then( [ 'Темы курсовых работ', 'table' ] )
        else(),
        [ 'Вопросы промежуточной аттестации', 'table' ],
        [ 'Вопросы для самостоятельного изучения', 'table' ],

        if( $дисциплина//кодВидаРабот/text() = '6' )then(  [ 'Темы контрольных работ', 'table' ])else(),
        [ 'МТО', 'table' ]
    ...
    <table>
      <row  id = 'fields'>
        ...
        <cell id="вопросыПромежуточнойАттестации">{ if( $автор/row[ @id = "tables" ]/cell[ @id = 'Вопросы промежуточной аттестации']/table//cell/text() != "" )then()else( 'В соответствии с ФОС.' ) }</cell>
         ...
         <cell id="темыКонтрольныхРабот">{ if( $автор/row[ @id = "tables" ]/cell[ @id = 'Темы контрольных работ']/table/row )then()else( 'Контрольные работы не предусмотрены.' ) }</cell>
      </row>
...
declare function ivgpu:заполнитьДокумент( $data, $mode ){
  ...
  let $templateID := 'de968e94-033e-4d71-9b92-cbd68a300072'
...
ivgpu:блокДисциплины(
  ...
  return
    ...
    case 'Б1.Б'
      return
        "Дисциплина «" || $названиеДисциплины || "» (" || $кодДисциплины || ") относится к базовой части блока Дисциплины (модули) рабочего учебного плана."
    case 'Б1.В'
      return
        "Дисциплина «" || $названиеДисциплины || "» (" || $кодДисциплины || ") относится к дисциплинам вариативной части, формируемой участниками образовательных отношений блока Дисциплины (модули) рабочего учебного плана."
    case 'ФТД.В'
      return
        "Дисциплина «" || $названиеДисциплины || "»(" || $кодДисциплины || ") относится к блоку факультативных дисциплин."
    case 'Б2.В'
      return
