...
declare 
  %rest:path('/sandbox/ivgpu/generate/РПД.Титул/{$ID}/{$discID}/pdf')
function ivgpu:main( $ID, $discID ){

  let $fileName := 'titul24.RPD-titul.docx'
  ...
  let $file := 
    file:write-binary(
      ...
      fetch:binary(
        iri-to-uri( 'http://dbx.iro37.ru/sandbox/ivgpu/generate/РПД.Титул/' || $ID || '/' || $discID
        )
      ) 
    )
