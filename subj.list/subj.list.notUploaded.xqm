    ...
    return
      <ul><b><a href = "{ $ссылкаСтраницаДисцплины }">{ $i/@название/data() }</a></b>{
        ...
        let $href := 
          ...
          $программа/Файл/@ID/data() || '/' ||
          $программа/Дисциплины/Дисциплина
          [ @Название = $i/@название/data() ]/@КодДисциплины/data() || 
          '/comp'
        ...
        let $отвественныйПреподаватель :=
          ivgpu:ответственныйПреподаватель(
            ...
            normalize-space( $i/@название/data() )
          )
...
function ivgpu:ответственныйПреподаватель(
  ...
  return
     if( $дисциплина[ tokenize( normalize-space( Код_направления/text() ), ',') = $кодНаправления ] )
     then(
       $дисциплина[ tokenize( normalize-space( Код_направления/text() ), ',') = $кодНаправления ][ 1 ]/Преподаватель/text()
     )
     else(
       if( $дисциплина[ not( Код_направления/text() ) ]/Преподаватель/text() )
       then(
         $дисциплина[ not( Код_направления/text() ) ][ 1 ]/Преподаватель/text()
       )
       else(
         'Не закреплена'
       )
