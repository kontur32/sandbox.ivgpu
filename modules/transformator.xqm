...
for $План in $Планы
let $КодАктивногоПлана := $План/b:Планы/@КодАктивногоООП
let $OOP := $План/b:ООП/b:ООП[ @Код = $КодАктивногоПлана  ]
return
  <Программа КодНаправления = '{ $План/b:ООП/@Шифр }' НазваниеНаправления = '{ $План/b:ООП/@Название  }' НазваниеПрофиля = '{ $OOP/@Название}' Кафедра = '{ $План/b:Планы/@КодПрофКафедры }' Год = '{ $План/b:Планы/@ГодНачалаПодготовки }' КодФормыОбучения = '{ $План/b:Планы/@КодФормыОбучения }'>
    <Файл DETAIL__URL = '{ $План/parent::*/parent::*/@DETAIL__URL }' ID = '{ $План/parent::*/parent::*/@ID}' />
    <Дисциплины>
      {
        for $i in $План/b:ПланыСтроки[ @ТипОбъекта = '2' and ( @КодООП = $План/b:ООП/@Код or @КодООП = КодАктивногоПлана ) ]
        let $discComp := $План/b:ПланыКомпетенцииДисциплины[ @КодСтроки = $i/@Код ]/@Код
        return
          <Дисциплина КодДисциплины = '{ $i/@ДисциплинаКод }' Название = '{ $i/@Дисциплина }' КодКафедры = '{ $i/@КодКафедры}' ЗЕТ = '{$i/@ЗЕТфакт}'>
            {
              for $i in $План/b:ПланыКомпетенции [ @Код/data() = $discComp ]
              return
                <Компетенция ШифрКомпетенции = '{ $i/@ШифрКомпетенции }' Название = '{ $i/@Наименование }'/>
            }
          </Дисциплина>
      }
    </Дисциплины>
